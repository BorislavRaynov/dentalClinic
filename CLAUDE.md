...
- `patient` — `Patient` model with auto-generated `patient_id` (format `PT-XXXXXXXX`, unique, non-editable), `address`, `date_of_birth`, and `sex` (choices M/F/O). `ManyToManyField` to `Treatment` (legacy, retained for backward compatibility; see below). `Patient.delete()` clears treatments first. Owns six related clinical models: three via FK + CASCADE (`MedicalCondition` with optional `code` field for diagnoses; related_name `medical_conditions`, includes status/notes; `Allergy` related_name `allergies`, severity/reaction; `MedicalHistoryEntry` related_name `medical_history_entries`, timestamped notes); and three for the electronic medical record hub (`ClinicalNote` — dentist-authored, timestamped `created_at`, `body`, `author` FK to `DentistUser` SET_NULL, related_name `clinical_notes`; `Prescription` — `medication`, `dosage`, `instructions`, `date_prescribed`, `prescribed_by` FK SET_NULL, related_name `prescriptions`; `TreatmentRecord` — performed-treatment history, FK to `Treatment` `on_delete=PROTECT`, optional FK to `Appointment` SET_NULL, `performed_by` FK SET_NULL, nullable `date_performed`, `tooth`, `notes`, related_name `treatment_records`). `PatientMedicalRecordView` (DetailView) serves as a comprehensive medical-record hub listing all six clinical sections, each with scoped Create/Update/Delete views. Author, prescriber, and performer fields are auto-set from the logged-in dentist in the view layer, not the form. **Important:** `TreatmentRecord` is the real performed-treatment history (backfilled from the legacy `Patient.treatment` M2M via reversible data migration `0007_backfill_treatment_records`); deprecating the M2M in favor of `TreatmentRecord` is a planned future slice. Attached documents (images, files) are deferred to Phase 2 (requires file-storage infrastructure). Patient catalogue search (via `PatientListView`) matches across name, patient_id, email, and phone (demographic fields only; clinical fields excluded from search).
